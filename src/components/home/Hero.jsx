...
export default function Hero() {
  return (
    <section
      id="hero"
      className="flex flex-col items-center justify-center px-12 lg:flex-row xl:gap-x-10 lg:py-10 w-full"
    >
      {/* IMAGE SECTION */}
      ...
      <div className="flex flex-col text-center lg:text-left w-full lg:w-3/4 xl:w-1/2 sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-2xl gap-y-5">
        ...
        <p className="text-xl xl:text-2xl">
          Explore palettes, design gradients, and be inspired by thousands of
          color combinations.
        </p>
        <div className="flex flex-col lg:flex-row gap-3 md:gap-x-4 justify-center lg:justify-start">
          <NavLink
            to={"/generate"}
            className="bg-[#0075de] hover:bg-[#005BAB] px-6 lg:px-12 py-4 rounded-xl text-white text-base lg:text-lg"
          >
            Generate Now!
          </NavLink>
          <NavLink
            to={"/palettes"}
            className="border cursor-pointer border-[#b4b4b4] hover:border-black px-6 lg:px-12 py-4 rounded-xl text-black text-base lg:text-lg"
          >
            Discover Palettes
          </NavLink>
        </div>
